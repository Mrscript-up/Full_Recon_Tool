...
def run_Tec_tool(targett):
    ...
    os.makedirs("tec_Reports", exist_ok=True)

    
    safe_name = re.sub(r'^https?://', '', target)
    safe_name = re.sub(r'[\\/:*?"<>|]+', '_', safe_name).strip('_') or "target"

    with open(f"tec_Reports/Tec_{safe_name}.html", "w", encoding="utf-8") as f:
        f.write(html_content)

    print(f"\n[+] HTML report saved in tec_Reports/Tec_{safe_name}.html")
